const checkTurnstile = ({ page }) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            let elements = await page.$$('[name="cf-turnstile-response"]');
            if (elements.length <= 0) {
                elements = await page.$$('input[type="hidden"][name="captcha"]');
            }
            if (elements.length <= 0) {
                ...
                for (const item of coordinates) {
                    try {
                        ...
                        console.log("Real Puppeteer Turnstile is cjs on1: " + x + " " + y);
            ...
            for (const element of elements) {
                try {
                    ...
                    console.log("Real Puppeteer Turnstile is cjs on2: " + x + " " + y);
                    await page.mouse.click(x, y);
                } catch (err) { }
            }
